...
public class ServerHandler extends SimpleChannelUpstreamHandler {
	...
	// Code changed by abhi
	// We check the routing.enabled value in conf file
	// if it is false then we do our usual flow if it is
	// true then the server acts as client and request is 
	...
	public void handleMessage(eye.Comm.Request req, Channel channel) {
		...
		if(req.getBody().hasImage())
		{
			//if (!Server.getRoutingInfo(req.getHeader().getRoutingId().getNumber()))
			if(req.getBody().getImage().getTag().equals(Server.getRoutingInfo(req.getHeader().getRoutingId().getNumber())))
			{
				// processing is deferred to the worker threads
				logger.info("Request is being serviced by this server");
				queueInstance(channel).enqueueRequest(req);
			}
			// code added by Team illuminati starts
			else
			{
				logger.info("Request needs to be routed to different Server");
				queueInstance(channel).enqueueRouteRequest(req);
				
			}
		}
		else if(req.getBody().hasUser())
		{
			//if (!Server.getRoutingInfo(req.getHeader().getRoutingId().getNumber()))
			if(req.getBody().getUser().getTag().equals(Server.getRoutingInfo(req.getHeader().getRoutingId().getNumber())))
			{
				// processing is deferred to the worker threads
				logger.info("Request is being serviced by this server");
				queueInstance(channel).enqueueRequest(req);
		...
		else if(req.getBody().hasFinger())
		{
			//if (!Server.getRoutingInfo(req.getHeader().getRoutingId().getNumber()))
			if(req.getBody().getFinger().getTag().equals(Server.getRoutingInfo(req.getHeader().getRoutingId().getNumber())))
			{
				// processing is deferred to the worker threads
				logger.info("Request is being serviced by this server");
				queueInstance(channel).enqueueRequest(req);
